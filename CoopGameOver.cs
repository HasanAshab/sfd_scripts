...
public void OnStartup()
{
    ...
    // Just access it to initialize - no need to set anything
...
public void CheckForHumanPlayers(TriggerArgs args)
{
    ...
    // Check if SuperDS.cs is in the middle of splitting P2
    IScriptStorage storage = Game.GetSharedStorage("SuperDSCoopSync");
    bool p2IsSplitting = false;
    float splitEndTime = 0;
    
    // Try to get splitting status from shared storage
    if (storage.TryGetItemBool("P2_SPLITTING", out p2IsSplitting) && p2IsSplitting)
    {
        // P2 is splitting - check if split time hasn't expired
        if (storage.TryGetItemFloat("P2_SPLIT_TIME", out splitEndTime))
        {
            if (Game.TotalElapsedGameTime < splitEndTime)
            {
                // P2 is still in splitting process - don't restart yet
                return;
            }
        }
    }
