...
{
    // Store the dead player's info for the split
    Vector2 spawnPos = deadPlayer.GetWorldPosition();
    ...
    // Trigger split after a short delay
    IObjectTimerTrigger splitTimer = (IObjectTimerTrigger)Game.CreateObject("TimerTrigger");
    splitTimer.SetIntervalTime(100); // 100ms delay
    splitTimer.SetRepeatCount(1);
    
    if (splitLevel == 1)
    {
        // Store split info for first split
        StoreSplitInfo(spawnPos, team, profile, user, wasHuman, 1);
        splitTimer.SetScriptMethod("SplitP2First");
    }
    else if (splitLevel == 2)
    {
        // Store split info for second split
        StoreSplitInfo(spawnPos, team, profile, user, wasHuman, 2);
...
private void StoreSplitInfo(Vector2 spawnPos, PlayerTeam team, IProfile profile, IUser user, bool wasHuman, int splitLevel)
{
