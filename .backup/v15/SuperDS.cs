...
public void OnPlayerDeath(IPlayer player, PlayerDeathArgs args)
{
    ...
    if (p2 != null && player.UniqueID == p2.UniqueID && !p2HasDied && !p2HasSplit)
    {
        ...
        splitTimer.SetIntervalTime(2000); // 1 second delay
...
public void SplitP2(TriggerArgs args)
{
    ...
    IPlayer oldP2 = p2;
    IPlayer newP2 = Game.CreatePlayer(oldP2.GetWorldPosition());
    newP2.SetTeam(oldP2.GetTeam());
    newP2.SetUser(oldP2.GetUser());
    newP2.SetProfile(oldP2.GetProfile());
    
    // Update p2 reference to the new player
    p2 = newP2;
