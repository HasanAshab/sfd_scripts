...
{
    // Only handle respawn for main players (P1-P8), not spawned ones
    if (
        !spawnedRookieIds.Contains(player.UniqueID) && 
        !spawnedCaptainIds.Contains(player.UniqueID) && 
        !spawnedArtilleryIds.Contains(player.UniqueID) &&
        !spawnedDroneIds.Contains(player.UniqueID))
    {
        ...
        if (colonel != null && !colonel.IsDead && !gameEnded)
        {
            ...
            if (playerRespawnTimers.ContainsKey(player.UniqueID))
            {
                ...
                // Also remove from dead players to prevent duplicates
                deadPlayersAwaitingRespawn.Remove(player.UniqueID);
            }
...
public void RespawnPlayer(TriggerArgs args)
{
    ...
    // Find the first timer that should be processed (the one that just expired)
    // Since we can't identify which specific timer called this method,
    // we'll process the first available one and remove it
    
    if (playerRespawnTimers.Count == 0) return;
    
    // Get the first timer entry
    var firstTimer = playerRespawnTimers.First();
    int playerID = firstTimer.Key;
    IObjectTimerTrigger timer = firstTimer.Value;
    
    // Remove the timer immediately to prevent duplicate processing
    ...
    if (deadPlayersAwaitingRespawn.ContainsKey(playerID))
    {
        ...
        // Remove the dead player info since we've processed it
