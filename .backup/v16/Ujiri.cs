        ...
        pakhiBot.SetProfile(new IProfile()
{
    ...
    if (psythicBot != null)
    {
        ...
        // Don't set bot behavior immediately - will be set after 2 seconds via timer
        ...
        // Set bot behavior after 2 seconds using timer
        IObjectTimerTrigger psythicBehaviorTimer = (IObjectTimerTrigger)Game.CreateObject("TimerTrigger");
        psythicBehaviorTimer.SetIntervalTime(2000); // 2 seconds
        psythicBehaviorTimer.SetRepeatCount(1); // Only once
        psythicBehaviorTimer.SetScriptMethod("SetPsythicBotBehavior");
        psythicBehaviorTimer.Trigger();
    }
        ...
        foreach (PlayerMeleeHitArg hitArg in args)
        {
            if (hitArg.HitObject != null && hitArg.IsPlayer)
            {
                ...
                if (hitPlayer != null && !hitPlayer.IsDead && hitPlayer.GetTeam() != psythicBot.GetTeam())
                {
                    ...
                    {
                        // 70% chance to stun enemy for 2 seconds (only if not gibbed)
                        int stunRoll = random.Next(0, 100);
                        if (stunRoll < 70)
                        {
                            StunPlayer(hitPlayer, 2000); // 2 seconds stun
                        }
                    }
                }
            }
...
public void SetPsythicBotBehavior(TriggerArgs args)
{
    // Set Psythic's bot behavior after 2 second delay
    if (psythicBot != null && !psythicBot.IsDead)
    {
