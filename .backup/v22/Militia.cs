...
public void CheckForWinner(TriggerArgs args)
{
    ...
    IPlayer[] mainPlayers = allPlayers.Where(p => !spawnedRookieIds.Contains(p.UniqueID) && 
                                                  ...
                                                  !spawnedArtilleryIds.Contains(p.UniqueID)).ToArray();
...
public void IdentifyColonels(TriggerArgs args)
{
    ...
    {
        if (player.IsBot && !spawnedRookieIds.Contains(player.UniqueID) && !spawnedCaptainIds.Contains(player.UniqueID) && !spawnedArtilleryIds.Contains(player.UniqueID))
        {
...
private void SpawnArtillery(PlayerTeam team)
{
    ...
    if (artillery != null)
    {
        ...
        artillery.GiveWeaponItem(WeaponItem.GRENADES);
