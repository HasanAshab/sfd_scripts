...
private Dictionary<int, int> playerSplitLevels = new Dictionary<int, int>(); // Track individual player split levels
...
public void OnStartup()
{
    ...
    // Initially enable auto victory - will be disabled when P2 splits
...
public void OnPlayerDeath(IPlayer player, PlayerDeathArgs args)
{
    ...
    {
        // Disable auto victory when P2 or split players are involved
        Game.AutoVictoryConditionEnabled = false;

        // Check if player was gibbed (completely destroyed) using the IsGibbed method
        bool wasGibbed = IsGibbed(player);
        
        if (wasGibbed)
        {
            // Player was gibbed - no split, just remove from tracking and check victory
            if (p2SplitPlayers.Contains(player))
            {
                p2SplitPlayers.Remove(player);
            }
            CheckAndEnableVictoryCondition();
            return;
        }

        // Get or initialize this player's split level
        int playerSplitLevel = 0;
        if (playerSplitLevels.ContainsKey(player.UniqueID))
        {
            playerSplitLevel = playerSplitLevels[player.UniqueID];
        }
        
        // Determine split based on individual player's split level
        if (player.UniqueID == p2.UniqueID && !p2HasDied)
        {
            // Original P2 died for the first time - first split
            p2HasDied = true;
            p2SplitLevel = 1; // Global split level for tracking
            TriggerP2Split(player, 1);
        }
        else if (p2SplitPlayers.Contains(player) && playerSplitLevel < 2)
        {
            // A first-split player died and hasn't split twice yet - second split
            playerSplitLevels[player.UniqueID] = 2; // Mark this specific player as having split twice
            TriggerP2Split(player, 2);
        }
        else
        {
            // Player died but won't split - check if we should re-enable victory
            CheckAndEnableVictoryCondition();
        }
    }
    else
    {
        // Non-P2 player died - check if we should re-enable victory
        CheckAndEnableVictoryCondition();
    }
}
...
{
    // Store the dead player's info for the split (but not position - we'll get that later)
    PlayerTeam team = deadPlayer.GetTeam();
    IProfile profile = deadPlayer.GetProfile();
    IUser user = deadPlayer.GetUser();
    bool wasHuman = !deadPlayer.IsBot;

    // Remove the dead player from tracking
    if (p2SplitPlayers.Contains(deadPlayer))
    {
        p2SplitPlayers.Remove(deadPlayer);
    }
    
    // Signal to CoopGameOver.cs that P2 is about to split (prevent restart)
    IScriptStorage storage = Game.GetSharedStorage("SuperDSCoopSync");
    storage.SetItem("P2_SPLITTING", true);
    storage.SetItem("P2_SPLIT_TIME", Game.TotalElapsedGameTime + 2500); // 2.5s buffer
    
    // Trigger split after a 2-second delay for better animation
    IObjectTimerTrigger splitTimer = (IObjectTimerTrigger)Game.CreateObject("TimerTrigger");
    splitTimer.SetIntervalTime(2000); // 2 seconds delay
    splitTimer.SetRepeatCount(1);
    
    if (splitLevel == 1)
    {
        // Store split info for first split (without position, store player reference)
        StoreSplitInfo(Vector2.Zero, team, profile, user, wasHuman, 1, deadPlayer);
        splitTimer.SetScriptMethod("SplitP2First");
    }
    else if (splitLevel == 2)
    {
        // Store split info for second split (without position, store player reference)
        StoreSplitInfo(Vector2.Zero, team, profile, user, wasHuman, 2, deadPlayer);
...
// Temporary storage for split information
private Vector2 tempSpawnPos;
private PlayerTeam tempTeam;
private IProfile tempProfile;
private IUser tempUser;
private bool tempWasHuman;
private int tempSplitLevel;
private IPlayer tempDeadPlayer; // Store player reference to get current position at split time

private void StoreSplitInfo(Vector2 spawnPos, PlayerTeam team, IProfile profile, IUser user, bool wasHuman, int splitLevel, IPlayer deadPlayer)
{
    tempSpawnPos = spawnPos; // Will be updated at split time
    tempTeam = team;
    tempProfile = profile;
    tempUser = user;
    tempWasHuman = wasHuman;
    tempSplitLevel = splitLevel;
    tempDeadPlayer = deadPlayer; // Store player reference
}

public void SplitP2First(TriggerArgs args)
{
    if (p2HasSplit) return; // Prevent multiple first splits
    
    // Get shared storage for communication with CoopGameOver.cs
    IScriptStorage storage = Game.GetSharedStorage("SuperDSCoopSync");
    
    // Check if the dead player was gibbed during the 2-second delay
    if (tempDeadPlayer != null && IsGibbed(tempDeadPlayer))
    {
        // Player was gibbed during delay - no split, clear splitting flag and check victory conditions
        storage.SetItem("P2_SPLITTING", false);
        CheckAndEnableVictoryCondition();
        return;
    }
    
    // Get current position of the dead player (after 2 seconds)
    Vector2 spawnPos = tempDeadPlayer != null ? tempDeadPlayer.GetWorldPosition() : Vector2.Zero;
    
    // First split: 1 human + 2 bots (using stored info)
    PlayerTeam team = tempTeam;
    IProfile profile = tempProfile;
    IUser user = tempUser;
    ...
    // Initialize split level for the new P2
    playerSplitLevels[newP2.UniqueID] = 1;
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        // Initialize split level for the bot
        playerSplitLevels[botPlayer.UniqueID] = 1;
    }
    
    p2HasSplit = true;
    
    // Clear splitting flag - split is complete
    storage.SetItem("P2_SPLITTING", false);
    
    // Keep auto victory disabled while P2 split players exist
    Game.AutoVictoryConditionEnabled = false;
}
...
public void SplitP2Second(TriggerArgs args)
{
    ...
    // Second split: Only the dead player splits into 3 smaller versions (0.7x size)
    PlayerTeam team = tempTeam;
    IProfile profile = tempProfile;
    IUser user = tempUser;
    bool wasHuman = tempWasHuman;
    
    // Create 3 smaller versions of the dead player
    for (int i = 0; i < 3; i++)
    {
        IPlayer newPlayer = Game.CreatePlayer(spawnPos);
        newPlayer.SetTeam(team);
        newPlayer.SetProfile(profile);
        
        if (wasHuman && i == 0)
        {
            // First copy is human (maintains user control)
            newPlayer.SetUser(user);
            
            // Update p2 reference to the human copy
            p2 = newPlayer;
        }
        else
        {
            // All other copies are bots (including when original was bot)
            ...
            // All bots always guard the single human P2 player (if exists)
            if (p2 != null && !p2.IsBot)
            {
        ...
        // Set 0.7x size for second split players
        PlayerModifiers newPlayerMods = newPlayer.GetModifiers();
        newPlayerMods.SizeModifier = 0.7f;

        newPlayerMods.RunSpeedModifier = 1.1f;
        newPlayerMods.SprintSpeedModifier = 1.1f;

        newPlayerMods.MaxHealth = (int)(newPlayerMods.MaxHealth * 0.5f);
        newPlayerMods.CurrentHealth = (int)(newPlayerMods.CurrentHealth * 0.5f);

        newPlayerMods.MeleeDamageTakenModifier *= 0.4f;
        newPlayerMods.ProjectileDamageTakenModifier *= 0.3f;

        newPlayerMods.MeleeForceModifier *= 0.7f;
        newPlayerMods.MeleeDamageDealtModifier *= 0.7f;
        newPlayer.SetModifiers(newPlayerMods);

        // Add to split players list
        p2SplitPlayers.Add(newPlayer);
        
        // Mark these as final split (level 2) - they cannot split again
        playerSplitLevels[newPlayer.UniqueID] = 2;
    }
...
{
    // Check if P2 or any P2 split players are still alive
    bool p2StillExists = false;
    PlayerTeam p2Team = PlayerTeam.Independent; // Default value
    
    // Check if original P2 is still alive and in game
    if (p2 != null && !p2.IsDead)
    {
        // Verify P2 is actually in the game (not removed)
        IPlayer[] allPlayers = Game.GetPlayers();
        foreach (IPlayer player in allPlayers)
        {
            if (player.UniqueID == p2.UniqueID)
            {
                p2StillExists = true;
                p2Team = player.GetTeam();
                break;
            }
        }
    }
    
    // Check if any P2 split players are still alive and in game
    if (!p2StillExists && p2SplitPlayers.Count > 0)
    {
        IPlayer[] allPlayers = Game.GetPlayers();
        foreach (IPlayer splitPlayer in p2SplitPlayers)
        {
            if (!splitPlayer.IsDead)
            {
                // Verify split player is actually in the game
                foreach (IPlayer player in allPlayers)
                {
                    if (player.UniqueID == splitPlayer.UniqueID)
                    {
                        p2StillExists = true;
                        p2Team = player.GetTeam();
                        break;
                ...
                if (p2StillExists) break;
            }
        }
    }
    
    // If no P2 or P2 split players exist, re-enable auto victory
    if (!p2StillExists)
    {
        Game.AutoVictoryConditionEnabled = true;
        return;
    ...
    // Check if only P2's team remains on the map (excluding NONE team)
    bool onlyP2TeamRemains = true;
    IPlayer[] currentPlayers = Game.GetPlayers();
    
    foreach (IPlayer player in currentPlayers)
    {
        if (!player.IsDead && player.GetTeam() != PlayerTeam.Independent)
        {
            // If there's a living player from a different team than P2's team
            if (player.GetTeam() != p2Team)
            {
                onlyP2TeamRemains = false;
                break;
            }
        }
    }
    
    // If only P2's team remains (excluding NONE/Independent team), enable auto victory
    if (onlyP2TeamRemains)
    {
        Game.AutoVictoryConditionEnabled = true;
    }
}
...
public void OnUpdate(float elapsed)
{
    ...
    if (p1VulnerableUntil > 0 && currentTime < p1VulnerableUntil && !p1.IsDead)
    {
        ...
        Vector2 p1Position = p1.GetWorldPosition();
        Game.PlayEffect(EffectName.SmokeTrail, p1Position);
    }
    ...
    // Reset guard targets for P2 split individuals to ensure they always guard the main human P2
    if (p2 != null && !p2.IsBot && !p2.IsDead && p2SplitPlayers.Count > 0)
    {
        foreach (IPlayer splitPlayer in p2SplitPlayers)
        {
            if (splitPlayer != null && splitPlayer.IsBot && !splitPlayer.IsDead)
            {
                // Ensure all P2 bot split players guard the main human P2
                splitPlayer.SetGuardTarget(p2);
            }
        }
    }
    
    // Periodically check if victory conditions should be re-enabled
    // This ensures proper victory handling even if some events are missed
    if (p2HasSplit)
    {
        CheckAndEnableVictoryCondition();
    }
...
public bool IsGibbed(IPlayer player) {
    IPlayer[] allPlayers = Game.GetPlayers();
